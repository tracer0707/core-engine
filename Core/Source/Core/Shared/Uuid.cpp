...
namespace Core
{
	...
	{
		if (ch >= static_cast<char>('0') && ch <= static_cast<char>('9')) return static_cast<unsigned char>(ch - static_cast<char>('0'));
		if (ch >= static_cast<char>('a') && ch <= static_cast<char>('f')) return static_cast<unsigned char>(10 + ch - static_cast<char>('a'));
		if (ch >= static_cast<char>('A') && ch <= static_cast<char>('F')) return static_cast<unsigned char>(10 + ch - static_cast<char>('A'));
		return 0;
	}

	bool Uuid::isHex(const char ch)
	{
		return (ch >= static_cast<char>('0') && ch <= static_cast<char>('9')) || (ch >= static_cast<char>('a') && ch <= static_cast<char>('f')) ||
	...
	Uuid Uuid::create()
	{
		...
		std::array<uint8_t, 16> bytes = {{bytes.byte0, bytes.byte1, bytes.byte2, bytes.byte3, bytes.byte4, bytes.byte5, bytes.byte6, bytes.byte7,
	...
	bool Uuid::isValidUuid(std::string& str)
	{
