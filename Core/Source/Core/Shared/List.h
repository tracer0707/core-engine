...
namespace Core
{
	...
	class List
	{
		...
		public:
			List(std::initializer_list<T> init) : list(init) {}

			// Остальные конструкторы
			List() = default;
			List(size_t size) : list(size) {}
			List(size_t size, const T& value) : list(size, value) {}
	...
	template <typename T>
	inline void List<T>::set(int index, const T& value)
	{
		assert(index >= 0 && index < static_cast<int>(list.size()) && "List index out of bounds");
		list[index] = value;
	}
	...
	template <typename T>
	inline void List<T>::fill(const T& value)
	{
