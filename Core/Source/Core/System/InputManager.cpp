...
namespace Core
{
    ...
    {
        auto it =
            std::find_if(mouseMoveEvents.begin(), mouseMoveEvents.end(), [=](std::pair<Uuid, MouseEvent>& evt) -> bool { return evt.first == id; });

        if (it != mouseMoveEvents.end()) mouseMoveEvents.erase(it);
    }

    void InputManager::unsubscribeMouseDownEvent(Uuid id)
    {
        auto it = std::find_if(mouseDownEvents.begin(), mouseDownEvents.end(),
                               [=](std::pair<Uuid, MouseButtonEvent>& evt) -> bool { return evt.first == id; });

        if (it != mouseDownEvents.end()) mouseDownEvents.erase(it);
    }

    void InputManager::unsubscribeMouseUpEvent(Uuid id)
    {
        auto it =
            std::find_if(mouseUpEvents.begin(), mouseUpEvents.end(), [=](std::pair<Uuid, MouseButtonEvent>& evt) -> bool { return evt.first == id; });

        if (it != mouseUpEvents.end()) mouseUpEvents.erase(it);
    }

    void InputManager::unsubscribeMouseWheelEvent(Uuid id)
    {
        auto it =
            std::find_if(mouseWheelEvents.begin(), mouseWheelEvents.end(), [=](std::pair<Uuid, MouseEvent>& evt) -> bool { return evt.first == id; });

        if (it != mouseWheelEvents.end()) mouseWheelEvents.erase(it);
    }
    ...
    {
        auto it =
            std::find_if(keyDownEvents.begin(), keyDownEvents.end(), [=](std::pair<Uuid, KeyboardEvent>& evt) -> bool { return evt.first == id; });

        if (it != keyDownEvents.end()) keyDownEvents.erase(it);
    }

    void InputManager::unsubscribeKeyUpEvent(Uuid id)
    {
        auto it = std::find_if(keyUpEvents.begin(), keyUpEvents.end(), [=](std::pair<Uuid, KeyboardEvent>& evt) -> bool { return evt.first == id; });

        if (it != keyUpEvents.end()) keyUpEvents.erase(it);
    }
    ...
    void InputManager::mouseMoveEvent(int x, int y)
    {
        ...
        {
            if (it.second != nullptr) it.second(x, y);
        }
    }
    ...
    void InputManager::mouseDownEvent(MouseButton mb, int x, int y)
    {
        ...
        {
            if (it.second != nullptr) it.second(mb, x, y);
        }
    }
    ...
    void InputManager::updateKeys(void* event)
    {
        ...
        if (evt->type == SDL_KEYDOWN)
        {
            ...
            if (evt->key.repeat == 0)
            {
                ...
                {
                    if (kevent.second != nullptr) kevent.second(key);
                }
            }
        }
    ...
    void InputManager::updateMouse(void* window)
    {
        ...
        if (!cursorVisible) ImGui::SetMouseCursor(ImGuiMouseCursor_None);
        ...
        if (cursorLocked)
        {
            ...
            if (prevCursorLocked == cursorLocked) setMouseDirection(_x - (w / 2), _y - (h / 2));
