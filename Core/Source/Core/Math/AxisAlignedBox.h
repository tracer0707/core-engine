...
namespace Core
{
	class AxisAlignedBox
	{
		...
		public:
			...
			inline AxisAlignedBox(float mx, float my, float mz, float Mx, float My, float Mz)
				: mMinimum(glm::vec3(0, 0, 0)), mMaximum(glm::vec3(1, 1, 1))
			{
			...
			inline glm::vec3& getMinimum(void) { return mMinimum; }

			inline glm::vec3& getMaximum(void) { return mMaximum; }
			...
			{
				if (mExtent != Extent::EXTENT_FINITE) return;

				glm::vec3 centre = getCenter();
				glm::vec3 halfSize = getHalfSize();

				glm::vec3 newCentre = matrix * glm::vec4(centre, 1.0f);
				glm::vec3 newHalfSize(fabs(matrix[0].x) * halfSize.x + fabs(matrix[1].x) * halfSize.y + fabs(matrix[2].x) * halfSize.z,
									  fabs(matrix[0].y) * halfSize.x + fabs(matrix[1].y) * halfSize.y + fabs(matrix[2].y) * halfSize.z,
									  fabs(matrix[0].z) * halfSize.x + fabs(matrix[1].z) * halfSize.y + fabs(matrix[2].z) * halfSize.z);

				setExtents(newCentre - newHalfSize, newCentre + newHalfSize);
			}

			inline void setNull() { mExtent = Extent::EXTENT_NULL; }

			inline bool isNull(void) const { return (mExtent == Extent::EXTENT_NULL); }

			bool isFinite(void) const { return (mExtent == Extent::EXTENT_FINITE); }

			inline void setInfinite() { mExtent = Extent::EXTENT_INFINITE; }

			bool isInfinite(void) const { return (mExtent == Extent::EXTENT_INFINITE); }

			inline bool intersects(const AxisAlignedBox& b2) const
			{
				if (this->isNull() || b2.isNull()) return false;

				if (this->isInfinite() || b2.isInfinite()) return true;

				if (mMaximum.x < b2.mMinimum.x) return false;
				if (mMaximum.y < b2.mMinimum.y) return false;
				if (mMaximum.z < b2.mMinimum.z) return false;

				if (mMinimum.x > b2.mMaximum.x) return false;
				if (mMinimum.y > b2.mMaximum.y) return false;
				if (mMinimum.z > b2.mMaximum.z) return false;
			...
			bool intersects(const glm::vec3& v) const
			{
				...
				case Extent::EXTENT_FINITE:
					return (v.x >= mMinimum.x && v.x <= mMaximum.x && v.y >= mMinimum.y && v.y <= mMaximum.y && v.z >= mMinimum.z &&
							v.z <= mMaximum.z);
			...
			glm::vec3 getCenter(void) const
			{
				...
				return glm::vec3((mMaximum.x + mMinimum.x) * 0.5f, (mMaximum.y + mMinimum.y) * 0.5f, (mMaximum.z + mMinimum.z) * 0.5f);
			}
			...
			float distance(const glm::vec3& v) const { return sqrt(squaredDistance(v)); }

			bool contains(const AxisAlignedBox& other) const
			{
				if (other.isNull() || this->isInfinite()) return true;

				if (this->isNull() || other.isInfinite()) return false;
			...
			{
				if (this->mExtent != rhs.mExtent) return false;
