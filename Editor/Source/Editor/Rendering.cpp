...
namespace Editor
{
    void Rendering::renderGrid(Core::Renderer* renderer, const Core::VertexBuffer* buffer, Core::Material* material, Core::Camera* camera)
    {
        ...
        Core::Primitives::lines(renderer, buffer, material, view, proj, mtx, verts1.ptr(), verts1.count(), flags);

        if (verts2.count() > 0) Core::Primitives::lines(renderer, buffer, material, view, proj, mtx, verts2.ptr(), verts2.count(), flags);
    }
}